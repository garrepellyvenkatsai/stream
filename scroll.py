...
def main():
    ...
    if (options or (uploaded_file and visOption)):
        if options:
            base_dir = "X:\Technology\CoPilot\Embeddings\chatWithDoc"
            first_option = options[0]
            if first_option in st.session_state.files_dic:
                persist_dir = os.path.join(base_dir, st.session_state.files_dic[first_option])
            else:
                persist_dir = os.path.join(base_dir, globalFiles_dic[first_option])

            db = Chroma(persist_directory=persist_dir, embedding_function=embeddings)

            for option in options[1:]:
                if option in st.session_state.files_dic:
                    persist_dir = os.path.join(base_dir, st.session_state.files_dic[option])
                else:
                    persist_dir = os.path.join(base_dir, globalFiles_dic[option])
        ...
        with tab2:
            if options:
                base_dir = "X:\Technology\CoPilot\chatWithDocUserFiles"
                for option in options:
                    file_path = os.path.join(base_dir, option)
                    file_extension = os.path.splitext(option)[1].lower()
                    if file_extension == ".pdf":
                        show_pdf(file_path)
                    elif file_extension == ".docx":
                        show_docx(file_path)
